...
load_dotenv()
log_folder = "hotpot_qa/logs/react_cot"
if not os.path.exists(log_folder):
    os.makedirs(log_folder)

# Set up logging
# log_file = os.path.join(log_folder, f"react_cot_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.log")
log_file = os.path.join(log_folder, f"react_cot_logical.log")
    ...
    def webthink(self,idx=None, to_print=True):
        ...
        for i in range(1, 8):
            ...
            try:
                ...
                if thought_action == "content_filter":
                    done =True
                    r, info['em']=0, 0
                n_badcalls += 1
                n_calls += 1
...
if __name__=="__main__":
    logging.info("Hotpot_React_CoT initialized.")
    load_dotenv()
    ...
    for i in idxs[:100]:
        ...
        if info['answer']=='' or info['answer']=='unknown' or info['answer']=='unable to answer' or info['answer']=='Sorry, unable to find the information':
