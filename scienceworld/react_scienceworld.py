...
load_dotenv()
log_folder = "hotpot_qa/logs_sc/seed_try_1"
...
# log_file2 = os.path.join(log_folder, "sc_validations_data_10.log")
...
class ScienceWorld_ReAct():
    ...
    def webthink(self, taskIdx, numEpisodes,simplificationStr, args, logger1,seed, to_print=True):
        ...
        for episodeIdx in range(0, numEpisodes) : 
            ...
            while (userInputStr not in exitCommands) and (isCompleted is False):
                ...
                print(f"Output by gpt : {gpt_action_str}")
                pattern = r"Action:\s*(.*)"

                # Search for the pattern in the text
                match = re.search(pattern, gpt_action_str, re.IGNORECASE)
                if match is not None:
                    # logger2.info(f"Len of valid actions : {len(action_list)}")
                    # logger2.info(f"Valid actions : {action_list}")
                    action = match.group(1).strip()
                else : 
                    # logger2.info(f"Len of template actions : {len(templates_list)}")
                    # logger2.info(f"Template actions : {templates_list}")
                    ...
                    if match is not None:
                        ...
                        # Task 2
                        # valid_restricted_actions=random.sample(action_list,200)
                        gpt_action_str=self.run_gpt3(self.webthink_prompt + f"\n Generate a Thought and an Action where the action should be based on the pssoible actions and possible observations.\nPossible type of actions : {poss_actions} and OBJ can be taken from possible type of objects ; {poss_objects}\n ",model="gpt-4", stop=[f"\nObservation:"])
                        # logger2.info(f"Actions : {poss_actions}")
                        # logger2.info(f"Objects : {poss_objects}")
...
def main():
    ...
    for i in idxs[27:28]:
        ...
        r, info = Sc_ReAct.webthink(i, numEpisodes,simplificationStr, args, logger1,seed=seeds,to_print=True)
        file_handler1.close()
