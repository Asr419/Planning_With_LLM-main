...
def clean_str(p):
  try:
    result = p.encode().decode("unicode-escape").encode("latin1").decode("utf-8")
  except UnicodeDecodeError as e:
    # Handle the exception gracefully, e.g., by printing an error message
    print("UnicodeDecodeError occurred:", e)
    # If you want to ignore the problematic part, you can skip it or replace it with an empty string
    p = p[:e.start] + p[e.end:]
    result = p.encode().decode("unicode-escape").encode("latin1").decode("utf-8")
  return result
