...
def lats_search(args, task, idx, iterations=30, to_print=True):
    ...
    for i in range(iterations):
        ...
        if node.is_terminal and node.reward == 1:
            ...
            return node.state, node.value, all_nodes, node.reward, node.em, node.f1
        ...
        if terminal_node.reward == 1:
            ...
            return terminal_node.state, terminal_node.value, [], terminal_node.reward, terminal_node.em, terminal_node.f1
        ...
        if terminal_nodes_with_reward_1:
            ...
            return best_node.state, best_node.value, all_nodes, best_node.reward, best_node.em, best_node.f1
    ...
    best_child = max(all_nodes_list, key=lambda x: x.f1 if isinstance(x.f1, (int, float)) else 0.0)
    failed_trajectories = []
    ...
    if best_child is None:
        best_child = root
    return best_child.state, best_child.value, all_nodes, best_child.reward, best_child.em, best_child.f1
