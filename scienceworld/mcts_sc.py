...
def mcts_sc_search(args, env, task, idx, taskName,simplificationStr,iterations=30):
    ...
    log_folder = "hotpot_qa/logs_sc/mcts"
    if not os.path.exists(log_folder):
        os.makedirs(log_folder)
   
    log_file2 = os.path.join(log_folder, "mcts_template5.log")
    ...
    for i in range(iterations):
        ...
        backpropagate(node, value,logger2)
        all_nodes = [(node, node.score) for node in collect_all_nodes(root)]

        terminal_nodes_with_score_100 = [(node, node.value) for node in collect_all_nodes(root) if node.is_terminal and node.score == 100]
        if terminal_nodes_with_score_100:
            logger2.info(f"Terminal node with score 100 found at iteration {i + 1}")
            best_node = max(terminal_nodes_with_score_100, key=lambda x: x.score)
    ...
    if best_child.score == 100:
        ...
        print(f"Returnung best nod unsuucedful score : {best_child.score}")
        # print(f"Returnung best nod unsuucedful score : {best_child.value}")
        # print(f"Returnung best nod unsuucedful score : {best_child.depth}")
...
def generate_new_states(env,node, args, task, logger2):
    ...
    for action in sampled_actions:
        ...
        if action_line:
            ...
            new_node.is_terminal = cumulative_score == 100 or done
            new_node.reward = r
